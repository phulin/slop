...
rates: eight pretraining-collection source samples, one SmolTalk2 Mid sample,
...
- The currently sampled Phase 3 pretraining sources cover `dclm` (`35.486%`),
  ...
  `pes2o` (`1.724%`), `fw2-fra` (`1.607%`), `finemath` (`1.410%`), plus
  `stackexchange` (`0.333%`) of the extracted recipe. This is enough for a
  majority-coverage weighted
  proxy, but not enough to compute a production weighted feature-rate baseline
  across all recipe sources.
...
- Ran it with source maps
  ...
  `smollm3_pretrain_finemath_2k=finemath`. The current retained Tier-1 proxy
  covers `75.912%` of the extracted recipe and leaves `24.088%` unsampled.
- Covered-only weighted rates per 1k tokens are: `slop_lexicon` `0.410`,
  `rule_of_three_approx` `4.473`, `contrastive_negation` `0.397`,
...
| Feature | Weighted proxy covered-only | FineWeb-Edu | DCLM | FineWeb2 German | FineWeb2 Spanish | FineWeb2 French | FineMath | PES2O | StackExchange | Mid | SFT target | Chosen | Rejected | Chosen - Rejected | Pair BH q |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| `contrastive_negation` | 0.397 | 0.385 | 0.487 | 0.006 | 0.002 | 0.002 | 0.181 | 0.238 | 0.509 | 0.177 | 0.095 | 0.379 | 0.352 | +0.027 | 0.039 |
| `rule_of_three_approx` | 4.473 | 6.211 | 3.746 | 0.184 | 0.038 | 0.100 | 1.817 | 5.520 | 1.583 | 1.273 | 17.045 | 5.443 | 4.877 | +0.566 | 9.437e-13 |
| `slop_lexicon` | 0.410 | 0.536 | 0.354 | 0.047 | 0.005 | 0.088 | 0.150 | 0.716 | 0.327 | 0.050 | 0.419 | 1.609 | 1.293 | +0.316 | 3.367e-15 |
| `stock_closers` | 0.037 | 0.042 | 0.037 | 0.002 | 0.000 | 0.001 | 0.036 | 0.060 | 0.026 | 0.027 | 0.270 | 0.217 | 0.167 | +0.050 | 3.016e-04 |
| `stock_openers` | 0.030 | 0.027 | 0.037 | 0.000 | 0.000 | 0.000 | 0.065 | 0.000 | 0.069 | 0.000 | 0.106 | 0.571 | 0.334 | +0.237 | 0.161 |
| `stock_openers_closers` | 0.066 | 0.068 | 0.074 | 0.002 | 0.000 | 0.001 | 0.100 | 0.060 | 0.095 | 0.027 | 0.376 | 0.788 | 0.502 | +0.287 | 0.003 |
...
3. Expand SmolLM3 pretraining feature-rate coverage beyond the current
   `75.912%` retained Tier-1 recipe share covered by DCLM, FineWeb-Edu,
   FineWeb2 German, FineWeb2 Spanish, FineWeb2 French, FineMath, PES2O, and
   StackExchange.
   ...
   next directly sampleable high-impact source is FineWeb2 Italian (`1.062%`),
   followed by other FineWeb2 language shards, `infiwebmath` (`0.903%`), and
   `finemath-4plus` (`0.606%`). `stack-edu-Cpp` (`1.304%`) carries the same
