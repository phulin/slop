...
- The current bounded pretraining feature-rate samples cover `dclm`
  ...
  (`1.607%`), and `stackexchange` (`0.333%`). The remaining production
...
- Added a bounded `fw2-fra` source sample from `HuggingFaceFW/fineweb-2`,
  ...
  hash-reservoir scan.
- With DCLM, FineWeb2 German, FineWeb2 Spanish, FineWeb2 French, and PES2O
  included, the current retained Tier-1 proxy covers `74.502%` of the
  extracted recipe and leaves `25.498%` unsampled, so its covered-only rates
  are majority-coverage diagnostics rather than full-mixture estimates.
...
- SmolLM3 exact recipe source weights are now extracted from the published
  ...
  FineWeb2 French, and PES2O are now sampled. `stack-edu-Python` (`1.811%`) is
  the largest unresolved source, but `HuggingFaceTB/stack-edu` config `Python`
  exposes blob metadata rather than code text, so measuring it needs a
  blob-hydration path or another text-bearing mirror. The next directly
  sampleable high-impact source is `finemath` (`1.410%`).
