...
- **H3 (compounding).** Free-running emission rates exceed teacher-forced propensity predictions for register-locking features (lists, headers), i.e., slop is partly self-conditioning.
...
- Define **opportunity positions** O_f: token positions where the pattern *could* initiate (contrastive negation: post-clause-boundary positions; lists: post-colon/newline positions; lexical items: all positions, or POS-gated). Opportunity definitions are part of the feature spec and frozen before measurement.
...
Generate N = 8 completions per prompt on 5k held-out SFT prompts per checkpoint, at temperature ∈ {0.0, 0.7, 1.0}, top-p = 0.95, max 1024 tokens (fixed and reported). Measure realized per-opportunity feature rates in generations.
...
**Compute estimate:** teacher forcing ~30M held-out tokens × 4 checkpoints × 2 ladders (7B + 3B, so cheaper than two 7Bs) ≈ a few hundred A100-hours; generation 5k × 8 × 3 temps × 4 ckpts × 2 ladders ≈ similar. Both fit a single 8×A100 node over ~1–2 weeks with vLLM (generation) and a logits-capture harness (teacher forcing). Tier-1 first; Biber/Tier-3 reuse cached generations. The Think/RL Zero path comparison adds ~50% if pursued.
...
| 5–7 | Full Phase 2 grid on OLMo 3 Instruct ladder; compounding decomposition (Result B) |
